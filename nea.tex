...
unable to afford. A 14 day free trial option is also available but we were
unable to try it out at first, because an error would be displayed claiming the
email address entered had already been used previously. For a reason we are
unaware the free trial option eventually worked.
...
lot of flexibility. Additionally arrays can contain values of any type include
of differing types. This follows our philosophy of accepting as much code as
possible, something we have to do because of how loosely the language is
defined by OCR.
...
loops because we don't have support for \texttt{input} in tests. We plan on
adding the use of \texttt{input} in tests later, at which time we'll add a test
for \texttt{do}-\texttt{until} loops.
...
as it meant making our parser recursive which as we explored before is not easy
to implement.
...
\begin{table}
	\begin{center}
		\begin{tabular}{|l|l|S|}
			...
			\hline
			\multirowcell{2}{OCR Reference Language} & Web Assembly & 198.331 \\
			\cline{2-3}
			& Native & 102.902 \\
			\hline
			\multirowcell{2}{Python} & CPython & 54.422 \\
			\cline{2-3}
			& PyPy & 8.450 \\
			\hline
			\multirowcell{2}{Rust} & No optimisations & 8.422 \\
			\cline{2-3}
