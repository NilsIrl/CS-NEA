...
to be unresponsive after too many errors occuring. We decided to fix it by
restarting the worker whenever an error occurs and by disabling the action
buttons on the playground until the worker is ready to accept code.

\section{Evaluation}

% code from 2018 goes beyond page
% talk about website having issues after panics
% maybe the button can be disabled until the worker is ready
...
Listing \ref{lst:2018_Q1c} shows the source code used and
\autoref{fig:2018_Q1c} shows the result of running the source code.
...
However, whilst developing the project we realised it would be very hard to
test it so we developed a playground to test out the language. This playground
is fully functional and supports all the features of the language except the
ones that do not make sense on the web, such as file operations and the
\texttt{system} function. However a function that was originally planned was
the use of the \texttt{input()} function to accept input from the user.
Unfortunately this function does not work on the playground because of
JavaScript's and the web's asynchronous nature. It is not possible for our
synchronous web assembly module to wait for input from the user as that would
necessarily require our program to work asynchronously. In order to fix this
issue, there are two ways that we can think of. Firstly, convert our program to
be asynchronous. This is an option that is possible as rust's Future is
compatible with JavaScript Promises, the types that represent something that
...
interface. Another solution would be to use a channel between the worker and
the web page that is synchronous. This is possible with the
...
web page would depend on to communicate using a common language.
