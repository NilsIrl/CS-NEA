...
final_files.tex:
	git ls-files -- prog  | grep -v '\.gitignore' | grep -v 'package-lock.json' | grep -v rust-toolchain | cut -d '/' -f 2- | sed 's/.*\.\(.*\)/\\subsection{&} \\inputminted{\1}{.\/prog\/&}/' | sed -E 's/\{(input|output)\}/{text}/' | sed 's/{ast}/{rs}/' | awk -F '}' '{gsub(/_/,"\\_",$$1)}1' OFS=} >final_files.tex
